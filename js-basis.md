...
* arguments
  ...
  - 不论是否传递实参,arguments 也都存在
    ...
    > 在非严格模式下,arguments 和形参存在映射关系(一个改都会跟着改)  
    > 严格模式下 arguments 和形参的映射机制就切断了
...
> A||B:先验证 A 的真假，如果 A 为真，返回 A，如果 A 为假，返回 B  
...
- 元素.属性名=属性值
  > 原理是向元素对象的堆内存中添加了一个属性
- 元素.setAttribute(属性名,属性值)
  > 把属性信息写到了元素标签的结构上(在结构上可以看到的),并没有放到元素对象对应的堆内存中
  > 获取属性值
- 元素.属性名
  > 原理是从堆内存中获取到对应的属性值
- 元素.getAttribute(属性名)
